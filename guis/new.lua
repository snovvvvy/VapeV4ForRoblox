		...
		then
			...
			local changed = inputService.InputChanged:Connect(function(input)
				...
				then
					...
					tween:Tween(gui, uipallet.Tween, {
...
components = {
	...
	ColorSlider = function(optionsettings, children, api)
		...
		local function createSlider(name, gradientColor)
			...
			fill.Size = UDim2.fromScale(
				math.clamp(
					name == "Saturation" and optionapi.Sat
						or name == "Vibrance" and optionapi.Value
						or optionapi.Opacity,
					0.04,
					0.96
				),
				1
					...
					local changed = inputService.InputChanged:Connect(function(input)
						...
						then
							optionapi:SetValue(
								...
								name == "Saturation"
										and math.clamp(
											(input.Position.X - bkg.AbsolutePosition.X) / bkg.AbsoluteSize.X,
											0,
											1
										)
									or nil,
		...
		preview.MouseButton1Click:Connect(function()
			...
			valuebox.Text = math.round(text.R * 255)
				.. ", "
		...
		slider.InputBegan:Connect(function(inputObj)
			...
			then
				...
				local changed = inputService.InputChanged:Connect(function(input)
					...
					then
						optionapi:SetValue(
							math.clamp((input.Position.X - bkg.AbsolutePosition.X) / bkg.AbsoluteSize.X, 0, 1)
						)
					end
				end)
		...
		valuebox.FocusLost:Connect(function(enter)
			...
			if enter then
				...
				local suc, res = pcall(function()
					return tonumber(commas[1])
							and Color3.fromRGB(tonumber(commas[1]), tonumber(commas[2]), tonumber(commas[3]))
	...
	Slider = function(optionsettings, children, api)
		...
		fill.Name = "Fill"
		fill.Size =
			UDim2.fromScale(math.clamp((optionapi.Value - optionsettings.Min) / optionsettings.Max, 0.04, 0.96), 1)
		fill.Position = UDim2.new()
		...
		slider.InputBegan:Connect(function(inputObj)
			...
			then
				local newPosition =
					math.clamp((inputObj.Position.X - bkg.AbsolutePosition.X) / bkg.AbsoluteSize.X, 0, 1)
				optionapi:SetValue(
					math.floor(
						(optionsettings.Min + (optionsettings.Max - optionsettings.Min) * newPosition)
	...
	TextList = function(optionsettings, children, api)
		...
		textlist:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
			...
			local actualPosition = (
				textlist.AbsolutePosition
				- (api.Legit and mainapi.Legit.Window.AbsolutePosition or -guiService:GetGuiInset())
			) / scale.Scale
			window.Position = UDim2.fromOffset(actualPosition.X + 220, actualPosition.Y)
		end)
	...
	Toggle = function(optionsettings, children, api)
		...
		function optionapi:Color(hue, sat, val, rainbowcheck)
			if self.Enabled then
				...
				knobholder.BackgroundColor3 = rainbowcheck
						and Color3.fromHSV(mainapi:Color((hue - (self.Index * 0.075)) % 1))
					or Color3.fromHSV(hue, sat, val)
			end
		end
		...
		function optionapi:Toggle()
			...
			tween:Tween(knobholder, uipallet.Tween, {
				BackgroundColor3 = self.Enabled
						and (rainbowcheck and Color3.fromHSV(
							mainapi:Color((mainapi.GUIColor.Hue - (self.Index * 0.075)) % 1)
						) or Color3.fromHSV(mainapi.GUIColor.Hue, mainapi.GUIColor.Sat, mainapi.GUIColor.Value))
		...
		function optionapi:SetValue(max, value)
			...
			tween:Tween(fill, TweenInfo.new(0.1), {
				...
				Size = UDim2.fromScale(
					math.clamp(
						math.clamp(math.clamp(self.ValueMax / optionsettings.Max, 0.04, 0.96), 0.04, 0.96) - size,
						0,
						1
					),
					1
...
function mainapi:CreateGUI()
	...
	function categoryapi:CreateSettingsPane(categorysettings)
		...
		windowlist:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			...
			if not categoryapi.Buttons then
				warn("categoryapi.Buttons is nil")
				return
			end
			...
			slider.InputBegan:Connect(function(inputObj)
				...
				then
					local changed = inputService.InputChanged:Connect(function(input)
						...
						then
							local value =
								math.clamp((input.Position.X - holder.AbsolutePosition.X) / holder.AbsoluteSize.X, 0, 1)
							optionapi:SetValue(
		...
		slider.InputBegan:Connect(function(inputObj)
			...
			then
				local changed = inputService.InputChanged:Connect(function(input)
					...
					then
						optionapi:SetValue(
							...
							nil,
							math.clamp(
								math.round((input.Position.X - holder.AbsolutePosition.X) / scale.Scale / 27),
								1,
								7
							)
						)
				...
				optionapi:SetValue(
					...
					nil,
					nil,
					math.clamp(math.round((inputObj.Position.X - holder.AbsolutePosition.X) / scale.Scale / 27), 1, 7)
				)
			end
		end)
		rainbow.MouseButton1Click:Connect(function()
	...
	function categoryapi:CreateModule(modulesettings)
		...
		function moduleapi:SetBind(tab, mouse)
			...
			if #tab <= 0 then
				...
				bind.Size = UDim2.fromOffset(
					math.max(getfontsize(bindtext.Text, bindtext.TextSize, bindtext.Font).X + 10, 20),
					21
				)
			end
		end
		...
		function moduleapi:Toggle(multiple)
			...
			modulebutton.TextColor3 = (hovered or modulechildren.Visible) and uipallet.Text
		...
		local modulechildren_visible = false
		local modulechildren_collapsing = false
		local collapseId = 0

		local function getCanonicalSize()
			return UDim2.new(1, 0, 0, windowlist.AbsoluteContentSize.Y / scale.Scale)
		end
		...
		modulebutton.MouseLeave:Connect(function()
			hovered = false
			local open = modulechildren_visible or modulechildren_collapsing
			if not moduleapi.Enabled and not open then
				modulebutton.TextColor3 = color.Dark(uipallet.Text, 0.16)
				modulebutton.BackgroundColor3 = uipallet.Main
			end
			bind.Visible = #moduleapi.Bind > 0 or open
		end)
		modulebutton.MouseButton1Click:Connect(function()
			moduleapi:Toggle()
		end)
		modulebutton.MouseButton2Click:Connect(function()
			local oldPos = modulechildren.Position
			modulechildren.AnchorPoint = Vector2.new(0, 0)

			if not modulechildren_visible then
				collapseId += 1
				modulechildren_collapsing = false
				modulechildren_visible = true

				local targetSize = getCanonicalSize()
				modulechildren.Visible = true
				modulechildren.Size = UDim2.new(targetSize.X.Scale, targetSize.X.Offset, 0, 0)
				modulechildren.Position = oldPos
				tween:Tween(modulechildren, uipallet.Tween, { Size = targetSize })
			else
				collapseId += 1
				local myId = collapseId

				modulechildren_visible = false
				modulechildren_collapsing = true

				local targetSize = getCanonicalSize()
				tween:Tween(modulechildren, uipallet.Tween, {
					Size = UDim2.new(targetSize.X.Scale, targetSize.X.Offset, 0, 0)
				})

				task.delay(0.22, function()
					if collapseId ~= myId then return end
					modulechildren_collapsing = false
					modulechildren.Visible = false
					modulechildren.Size = getCanonicalSize()
				...
				if heldbutton and clickgui.Visible then
					...
					touchconnection = inputService.InputBegan:Connect(function(inputType)
						if inputType.UserInputType == Enum.UserInputType.Touch then
							...
							createMobileButton(
								moduleapi,
								inputType.Position + Vector3.new(0, guiService:GetGuiInset().Y, 0)
							)
		...
		if self.Expanded then
			tween:Tween(window, uipallet.Tween, {
				Size = UDim2.fromOffset(
					...
					math.min(41 + windowlist.AbsoluteContentSize.Y / scale.Scale, 601)
				),
			})
		else
			tween:Tween(tween:Tween(window, uipallet.Tween, {
				Size = UDim2.fromOffset(window.Size.X.Offset, 41),
			}))
		end
	end
...
function mainapi:CreateCategoryList(categorysettings)
	...
	icon.Name = "Icon"
	icon.Size = categorysettings.Size
	icon.Position = categorysettings.Position
		or UDim2.fromOffset(12, (categorysettings.Size.X.Offset > 20 and 13 or 12))
	icon.BackgroundTransparency = 1
	icon.Image = categorysettings.Icon
	icon.ImageColor3 = uipallet.Text
	icon.Parent = window
		...
		for i, v in (categorysettings.Profiles and mainapi.Profiles or self.List) do
			if categorysettings.Profiles then
				...
				local function bindFunction(self, tab, mouse)
					...
					if mouse then
						bindcovertext.Text = #tab <= 0 and "BIND REMOVED"
							or "BOUND TO " .. table.concat(tab, " + "):upper()
						bindcover.Size =
							UDim2.fromOffset(getfontsize(bindcovertext.Text, bindcovertext.TextSize).X + 20, 40)
						task.delay(1, function()
							bindcover.Visible = false
						end)
					end
	...
	function categoryapi:Expand()
		...
		tween:Tween(window, uipallet.Tween, {
			Size = UDim2.fromOffset(
				220,
				self.Expanded and math.min(51 + windowlist.AbsoluteContentSize.Y / scale.Scale, 611) or 45
			)
		})
		divider.Visible = children.CanvasPosition.Y > 10 and children.Visible
	end
...
if shared.vapedev then 
	general:CreateButton({
		...
		Tooltip = "Injects infinite yield"
	})
...
local targetinfocolortoggle = targetinfoobj:CreateToggle({
	...
	Function = function(callback)
		...
		if callback then
			targetinfobkg.BackgroundColor3 =
				Color3.fromHSV(targetinfocolor.Hue, targetinfocolor.Sat, targetinfocolor.Value)
...
function mainapi:UpdateTextGUI(afterload)
	...
	if textgui.Button.Enabled then
		...
		if VapeLabelCustom.Visible then
			local size =
				getfontsize(removeTags(VapeLabelCustom.Text), VapeLabelCustom.TextSize, VapeLabelCustom.FontFace)
			VapeLabelCustom.Size = UDim2.fromOffset(size.X, size.Y)
			VapeLabelCustom.Position = UDim2.new(
				right and 1 / VapeTextScale.Scale or 0,
				right and -size.X or 0,
				0,
				(VapeLogo.Visible and 32 or 8)
			)
		end
		...
		for i, v in mainapi.Modules do
			...
			if v.Enabled or table.find(found, i) then
				...
				if textguishadow.Enabled then
					...
					holderdrop.Position =
						UDim2.fromOffset(holdertext.Position.X.Offset + 1, holdertext.Position.Y.Offset + 1)
	...
	if textgui.Button.Enabled then
		VapeLogoGradient.Color = ColorSequence.new({
			...
			ColorSequenceKeypoint.new(
				1,
				textguigradient.Enabled and Color3.fromHSV(mainapi:Color((hue - 0.075) % 1))
					or Color3.fromHSV(hue, sat, val)
			),
		})
		...
		if i == "Main" then
			...
			for _, button in v.Buttons do
				if button.Enabled then
					button.Object.TextColor3 = rainbow
		...
		if button.Enabled then
			...
			if button.Object.UIGradient.Enabled then
				...
				button.Object.UIGradient.Color = ColorSequence.new({
					...
					ColorSequenceKeypoint.new(
						1,
						Color3.fromHSV(mainapi:Color((hue - ((button.Index + 1) * 0.025)) % 1))
					),
				})
			end
	...
	if not inputService:GetFocusedTextBox() and inputObj.KeyCode ~= Enum.KeyCode.Unknown then
		if mainapi.Binding then
			...
			mainapi.Binding:SetBind(
				checkKeybinds(mainapi.HeldKeybinds, mainapi.Binding.Bind, inputObj.KeyCode.Name) and {}
					or mainapi.HeldKeybinds,
				true
			)
			mainapi.Binding = nil
		end
