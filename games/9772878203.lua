				...
				repeat
					if entitylib.isAlive then
						...
						for _, v in collectionService:GetTagged("Pickup") do
							if not v:IsDescendantOf(lplr.Backpack) and not v:IsDescendantOf(lplr.Character) then
								...
								if handle then
									...
									success = false
									entitylib.character.RootPart.CFrame = handle.CFrame
									break
								end
							end
						end
...
run(function() 
    ...
    local oldt = {}
	local oldDialog
    local Dialog = roommate.Head.Dialog
    ...
    RoommateDialogue = vape.Categories.Minigames:CreateModule({
        ...
        Tooltip = "Change the roommate's initial dialogue."
    })
